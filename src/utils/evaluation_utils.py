...
def diff_in_diff(df_orig, y_column, x_column, event_date, event_name, offset_months=4):
    ...
    model = smf.ols(
        f'{x_column} ~ is_sports + {event_name} + interaction', data=df).fit()
    ...
    sns.barplot(x='Period', y=x_column, hue='Group', data=summary,
                palette=pallete_plt, hue_order=['Control', 'Sports'])
    plt.title(
    ...
    # Time Series Plot
    time_summary = df.groupby([y_column, "is_sports"])[
    ...
    weekly_sports = time_summary_copy[time_summary_copy['is_sports'] == 1].resample(
        'W').mean()
    weekly_control = time_summary_copy[time_summary_copy['is_sports'] == 0].resample(
        'W').mean()

    # Plot the resampled data
    plt.figure(figsize=(14, 7))
    plt.plot(weekly_sports.index, weekly_sports[x_column],
             label='Sports', marker='o', color=pallete_plt['Sports'])
    ...
    def plot_model_results_both_groups():
        ...
        plt.plot(df_model_visualize.index,
                 df_model_visualize[x_column], label='Actual')
        plt.plot(df_model_visualize.index,
                 df_model_visualize['fitted'], label='Fitted', linestyle='--')
        plt.axvline(event_date, color='red',
    ...
    def plot_model_results_group_and_control():
        ...
        plt.plot(df_control.index, df_control[x_column],
                 label='Control Actual', color=pallete_plt['Control'])
        plt.plot(df_control.index, df_control['fitted'], label='Control Fitted',
                 linestyle='--', color=pallete_plt_fitted['Control_fitted'])

        plt.plot(df_sports.index, df_sports[x_column],
                 label='Sports Actual', color=pallete_plt['Sports'])
        plt.plot(df_sports.index, df_sports['fitted'], label='Sports Fitted',
                 linestyle='--', color=pallete_plt_fitted['Sports_fitted'])
        plt.axvline(event_date, color='red',
                    linestyle='--', label='Event Date')
        plt.title(f'Difference in Difference Analysis: {event_name}')
        plt.xlabel('Date')
    ...
    def box_plot_pre_vs_post():
        ...
        sns.boxplot(x="Period", y=x_column, hue='is_sports',
    ...
    def difference_bars():
        ...
        control_diff = df_diff_visualize[(df_diff_visualize["is_sports"] == 0) & (df_diff_visualize.index < event_date)][x_column].mean(
        ) - df_diff_visualize[(df_diff_visualize["is_sports"] == 0) & (df_diff_visualize.index >= event_date)][x_column].mean()

        sports_diff = df_diff_visualize[(df_diff_visualize["is_sports"] == 1) & (df_diff_visualize.index < event_date)][x_column].mean(
        ) - df_diff_visualize[(df_diff_visualize["is_sports"] == 1) & (df_diff_visualize.index >= event_date)][x_column].mean()

        diff_in_diff_values = sports_diff - control_diff

        plt.figure(figsize=(10, 8))
        plt.bar(['Control', 'Sports'], [control_diff, sports_diff],
                color=[pallete_plt['Control'], pallete_plt['Sports']])
        plt.title(
